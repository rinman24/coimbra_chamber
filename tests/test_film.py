...
def test_get_film_prop():
    e_value = 1
    s_value = 0

    # Test e = 0 and s = 1 with '1/2'
    assert math.isclose(
        film.get_film_prop(e_value, s_value, '1/2'),
        1/2
    )

    # Test e = 0 and s = 1 with '1/3'
    assert math.isclose(
        film.get_film_prop(e_value, s_value, '1/3'),
        1/3
    )

    # Test raises ValueError
    with pytest.raises(ValueError) as err:
        film.get_film_prop(e_value, s_value, '1/4')
    ...
    temp_s = 290
    assert math.isclose(
        film.get_film_prop(temp_e, temp_s, '1/2'),
        295
    )

    # Test temp_e = 300 and temp_s = 290 with '1/3'
    assert math.isclose(
        film.get_film_prop(temp_e, temp_s, '1/3'),
        293.3333333333333
    )
