...
def cpm(film_state):
    """Use film_state to calculate cpm."""
    try:
        res = hap.HAPropsSI('cp_ha',
                            'P', film_state['P'],
                            'T', film_state['T'],
                            'Y', film_state['x1'])
        return round(res, 1)
...
def hfgs(temp):
    """Use temp to return h_fg."""
    try:
        hg = cp.PropsSI('H', 'T', temp, 'Q', 1, 'water')
        hf = cp.PropsSI('H', 'T', temp, 'Q', 0, 'water')
        return int(hg - hf)
...
class ExperimentalState:
    ...
    """
    def __init__(self, p, tch, tdp, tm, Lt):
        """
    ...
    @property
    def Lt(self):
        """Length of Stefan tube, [m]."""
        return self._Lt

    # ----------------------------------------------------------------------- #
    # Internal Methods
    # ----------------------------------------------------------------------- #
    def _change_state(self, p, tch, tdp, tm, Lt):
        """
...
class FilmProperties:
    ...
    Class Variables:
        references (`tuple` of `str`): References to use for evaluating the
            binary species diffusivity.
    ...
    def change_ref(self, ref):
        ...
        if ref in self.references:
            ...
            raise ValueError(
                '`ref` must be in {0}.'.format(self.references)
                )
        else:
            raise TypeError(
...
class ReferenceFilm:
    ...
    Class Variables:
        rules (`tuple` of `str`): Rules for evaluating the film properties.
        M1 (`float`): Molecular weight of water in kg/kmol
        M2 (`float`): Molecular weight of dry air in kg/kmol
    ...
    Instance Variables:
        ...
        x (`float`): Film reference film mole fraction of water vapor with no
            units.
        x1e (`float`): Environmental mole fraction of water vapor with no units.
        x1s_guess (`float`): Guess at interface mole fraction of water vapor
            with no units.
    """
    ...
    def __init__(self, exp_state, film_rule='one-third', ref='Mills'):
        ...
        self._x1e = None
        self._m1e = None
        self._x1s_guess = None
        self._m1s_guess = None
    ...
    @property
    def x1s_guess(self):
        """Guess at interface mole fraction of water vapor"""
        return self._x1s_guess
    ...
    def _eval_state(self):
        ...
        self._eval_x1e()
        self._eval_x1s_guess()
        self._x = self._use_film_rule(self._x1e, self._x1s_guess)
        self._m1e = self._x2m(self.x1e)
        self._m1s_guess = self._x2m(self.x1s_guess)

    def _eval_x1e(self):
        self._x1e = hap.HAPropsSI('Y',
                                  'T', self.t,
                                  'P', self.p,
                                  'Tdp', self._ExpState.tdp)

    def _eval_x1s_guess(self):
        self._x1s_guess = hap.HAPropsSI('Y',
                                        'T', self.ts_guess,
                                        'P', self.p,
                                        'RH', 1.0)
