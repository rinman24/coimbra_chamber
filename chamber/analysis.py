...
def postprocess(res):
    """
    This function will calculate abs(Q-0.5), find the index that corresponds
    to the minimum of that signal, use the index to fetch b and sig_b, convert
    b and sig_b to mddp and err, then return mddp, err, Q, and nu.
    """
    # I am given a results dataframe that contains all the information
    # First things first, I can add the attributes that I need:
    res['mddp'] = res['b']/A_TUBE
    res['err'] = res['sig_b']/A_TUBE*3
    res['signal_q'] = abs(res.Q - 0.5)

    # The next thing I need to do is find all of the target RHs in the test.
    # I will perform a post processing analysis for each of the RH's that
    # are valid.
    rh_values = set((i for i in res.RH))
    # Set up a list to hold the postprocessing results
    post = []
    for rh_target in rh_values:
        # Now that I have a target, I need to grab the signal_q from my target
        # that is a minimum.
        #answer_idx = res.loc[(res.RH == rh_target) & (res.signal_q > 0), 'signal_q'].idxmin()

        #!!!!
        # You need to check if the signal_q is ever positive here. 
        # If it is never positive, then there is no statistically sound fit.
        # This should be easy, look for signal_q[signal_q >= 0].
        # If you get an empty list, return None.
        # But does this have side effects?
        # It could trickle down into other subsequent function calls.
        # Debugger??
        # Turn plots off for debugging.
        #!!!!
        answer_idx = res.loc[(res.RH == rh_target) & (res.Q > 0.5), 'signal_q'].idxmin()
        # Now I have the idx for the answer I am interested in
        mddp = res.mddp[answer_idx]
        err = res.err[answer_idx]
        q = res.Q[answer_idx]
        nu = res.nu[answer_idx]
        chi2 = res.chi2[answer_idx]
        # I now have all of the info I need
        post.append([rh_target, mddp, err, q, nu, chi2])
    df_post = pd.DataFrame(post, columns=['RH', 'mddp', 'err', 'Q', 'nu', 'chi2'])
    return df_post
    
# It looks like there is an issue with the way I am selecting signal_q > 0,
# perhaps, there is a part of the test that doesn't have one over 0.5
# Am I limiting nu? or does the first part of this test not have a Q near 0.5...
# I may need to look at the data that I am using for the test, load it into a
# notebook and see what is going on.
# If there is no positive value near 0, then I should
#     1) return Null
#     2) return the one on the negative side....
# 
# But there should be one near 1 at the beginning, so I think i really need to
# look at the file I am using for testing.
# NEXT STEP, TRY TO LOAD THIS DATA FILE INTO YOUR NOTEBOOK AND SEE WHAT IS
# GOING ON... 

    # this returns a set of the valid RH's, which I can iterate through:

    # For each of the valid rh_values,
