...
def get_db_data(test_id):
    ...
    Creates a connection to the chamber MySql database and gets the mass and calculated
    relative humidity data using CoolProp.
...
def get_hum_index(rel_hum):
    ...
    Searches the full list of relative humidities rel_hum and returns the indicies where
    the relative humidity is a multiple of 10%.
    ...
    -------
    hum_index : list
        List of indicies where the relative humidity is a multiple of 10% in rel_hum.
    """
    hum_index = []
    for hum in range(int(round(min(rel_hum)*100, -1)), int(round(max(rel_hum)*100, -1)), 10):
        hum_index.append((np.abs(np.array(rel_hum) - hum/100).argmin()))
    if not hum_index:
        raise Exception('Can not resolve Relative Humidity, insufficient Humidity variation')
    if hum_index[-1] == len(rel_hum) - 1:
        hum_index = hum_index[:-1]
    return hum_index

def lin_reg(mass, start, end, tol=0.99):
    """Calculates and returns the slope, r^2, intercept for the argument relative humidity index.

    Calculaes slope, r^2, and the intercept for the linear regression that fits mass in the range
    defined by start and end, start being the center point of the regression and end being the width
    to either side which data will be taken. The function returns the slope, r^2 and intercept if the 
    r^2 is above the argument tolerance, tol, which defaults to 0.99. Returns None if the r^2 value is
    below the tolerance.
    ...
    tol : float
        The r^2 tolerance which dictates which slopes are retuned. Defaults to 0.99.
    ...
    None : None
        if r^2 < tol
    """
    slope, intercept, r_value, p_value, std_err = stats.linregress(range(len(mass))[start-end:start+end],mass[start-end:start+end])
    if r_value**2 >= tol:
...
def get_lin(mass, hum_idx, tol=0.99, get_one=False):
    ...
    tol : float
        The r^2 tolerance which dictates which slopes are retuned. Defaults to 0.99.
    get_one : boolean
        Boolean that dictates weither to get and return one linear regression or all.
...
def get_avg(mass, hum_idx, tol=0.99):
    """Calculates the average slope, intercept, and r^2 for a given r^2 tolerace level.

    Calculates and returns the average slope, intercept, and r_2 using all of the valid
    linear regressions under the argument r^2 tolerance level, tol.
...
def get_set_width(mass, hum_idx, idx):
    ...
    hum_idx : list
        List of indicies where the relative humidity is a multiple of 10% in rel_hum.
    idx : int
        The band width to one side that you wish to use to calulate the linear regression.
...
def get_change(slope, step=1):
    """Calculates the percent change for each step, size step (defaults to 1) in the inputed data, slope.
    ...
    change : list of floats
        List of percent changes. Note length is one less than argument data, slope.
    """
    change = []
...
def subplot_4_change(fig, ends, slope, hum_per_list):
    ...
    for key in hum_per_list:
        sub_4.plot(ends[key][1:501], get_change(slope[key][:501]), label="RH = {}%".format(key))
...
def subplot_6_lin(fig, mass, hum_dict, inter_99, slope_99, hum_per_list):
    """Adds the Evaporation Rate Linear Fits Graphed subplot to argument figure."""
    sub_6 = fig.add_subplot(3, 2, 6)
    sub_6.set_title('Ev-R W/ Fit R^2=0.99 [kg/s]')
    sub_6.plot(mass, color='y', label='Mass')
    for key in hum_per_list:
        sub_6.plot(range(hum_dict[key]-2000, hum_dict[key]+2000), inter_99[key] + slope_99[key]*range(hum_dict[key]-2000, hum_dict[key]+2000), label="RH = {}%".format(key))
    sub_6.legend(loc='center left', bbox_to_anchor=(1, 0.5))

def generate_summary(test_id):
    """Generates a summary of the data for the test whose TestID matches the argument test_id

    Generates a graphical and tabular summary of the data for the argument test_id's coresponing
    test. Neccessary values are calculated using helper functions. 6 subplots are produced and a
    tabular summary of information is printed to the console. The slopes using various methods of
    analysis, present in the tabular output, are returned.
    ...
    output_slopes : list of floats
        List of slopes for the relevant relative humidities calculated using various methods
        described in the tabular output.
    """
    ...
    rows = []
    for key in hum_per_list:
        rows.append([key, 'R^2 Limit = 0.99', slope_99[key], inter_99[key], r_99[key]])
        rows.append([key, 'R^2 Limit = 0.999', slope_999[key], inter_999[key], r_999[key]])
        rows.append([key, 'Average R^2 > 0.99', avg_slope_99[key], avg_inter_99[key], avg_r_99[key]])
        rows.append([key, 'Average R^2 > 0.999', avg_slope_999[key], avg_inter_999[key], avg_r_999[key]])
        rows.append([key, 'Band Width = 500', set_slope_250[key], set_r_250[key], set_inter_250[key]])
        rows.append([key, 'Band Width = 1000', set_slope_500[key], set_r_500[key], set_inter_500[key]])
    output_slopes = []
