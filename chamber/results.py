...
def get_avg_temp(cur_ch, test_id):
    ...
    This function searches the chamber database for the average TempObservations recorded
    for the given TestID.
    ...
    avg_temp : list of floats
        List of temperature observations averaged over the thermocouples each observation
        for the given TestID.
    """
...
def get_rel_hum(cur_ch, test_id, pressure=None, avg_temp=None, dew_point=None):
    """Use a TestID to get the relative humidity calcilated for observations in a given test.

    This finction uses the CoolProp module to calculate the realative humidity for each
    observation in the test mathcing the give TestID.
    ...
    test_id : int
        The TestID which normalized mass will be calculated and recorded for.
    pressure : list of floats
        The list of pressure observations for the given TestID. Dafaults to None.
    avg_temp : list of floats
        The list of avg_temp calculations for the given TestID. Defaults to None.
    dew_point : list of floats
        The list of dew_poiny observations for the given TestID. Defaults to None.

    Returns
    -------
    hum : list of floats
        List of relative humidity percentages for each observation in the given test.
    """
    hum = []
    if not pressure:
    ...
    for idx in range(len(pressure)):
        hum.append(HAPropsSI('RH', 'P', pressure[idx], 'T', avg_temp[idx], 'D', dew_point[idx]))
    return hum
